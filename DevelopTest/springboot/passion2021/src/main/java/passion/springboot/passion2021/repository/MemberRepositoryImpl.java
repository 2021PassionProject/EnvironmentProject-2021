...
public class MemberRepositoryImpl implements MemberRepository {
    ...
    public int update(Member member) {
        String query = "update member set email=?, pw=?, name=?, birth=?, phone=?, address=?, address2=? where id=MEMBER.ID";
        return jdbcTemplate.update(query, member.getEmail(), member.getPw(), member.getName(),member.getBirth(), member.getPhone(), member.getAddress(), member.getAddress2(), member.getId());
    }
